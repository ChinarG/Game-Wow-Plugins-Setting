...
local function SetTemplate(f, t, glossTex, ignoreUpdates, forcePixelMode, isUnitFrameElement)
	...
	if t == 'notrans' then glossTex = true end
	...
	if t == 'NoBackdrop' then
		...
		if E.db.general.transparent and t ~= 'notrans' then
			E:SetBackdrop(f, true, E.media.blankTex, (not E.twoPixelsPlease and E.mult) or E.mult*2)
		else
		...
		if not f.ignoreBackdropColors then
			if t == 'Transparent' or (E.db.general.transparent and t ~= 'notrans') then
		...
		if not E.PixelMode and not f.forcePixelMode and not E.db.general.transparent then
