...
local function filter_name(unit, name)
	if (unit:find('raid') or unit=='player') and E.db.unitframe.units.raid.name.filter_text ~= '' and E.db.unitframe.units.raid.name.filter_text ~= nil then
		name = gsub(name, E.db.unitframe.units.raid.name.filter_text, '')
	end
	return name
end
...
ElvUF.Tags.Events['status'] = 'UNIT_HEALTH PLAYER_UPDATE_RESTING UNIT_HEALTH_FREQUENT UNIT_MAXHEALTH UNIT_CONNECTION PLAYER_FLAGS_CHANGED'
ElvUF.Tags.Methods['status'] = function(u)
