			...
			if i <= frame.MAX_CLASS_BAR then
				...
				elseif E.myclass == "DEATHKNIGHT" and frame.ClassBar == "Runes" then
					...
					if bars[i].bg then
						...
						bars[i].bg:SetColorTexture(r * mu, g * mu, b * mu)
					end
...
function UF:UpdateClassBar(current, maxBars, hasMaxChanged)
	...
	for i=1, #self do
		if custom_backdrop then
			self[i].bg:SetColorTexture(custom_backdrop.r, custom_backdrop.g, custom_backdrop.b)
		else
			local r, g, b = self[i]:GetStatusBarColor()
			self[i].bg:SetColorTexture(r * .35, g * .35, b * .35)
		end
...
function UF:Construct_DeathKnightResourceBar(frame)
	...
	--	runes[i].Text:FontTemplate(nil, 12)
		UF:Configure_FontString(runes[i].Text)
	end
...
function UF:PostUpdateAdditionalPower(_, MIN, MAX, event)
	...
	if frame.USE_CLASSBAR and ((MIN ~= MAX or (not db.classbar.autoHide)) and (event ~= "ElementDisable")) then
		...
		local custom_backdrop = UF.db.colors.customclasspowerbackdrop and UF.db.colors.classpower_backdrop
		if custom_backdrop then
			self.bg:SetColorTexture(custom_backdrop.r, custom_backdrop.g, custom_backdrop.b)
		end
