...
function TT:GameTooltip_OnTooltipSetUnit(tt)
	...
	if self.db.showMount and unit ~= "player" and isPlayerUnit then
		for i = 1, 40 do
...
--	end
	...
	if(not UnitIsPlayer(unit)) and unitreaction then
		...
		tt:AddDoubleLine(format("%s:", REPUTATION), format("|cff%02x%02x%02x%s|r", fcolor.r * 255, fcolor.g * 255, fcolor.b * 255, GetText("FACTION_STANDING_LABEL"..unitreaction, gender)))
	end
