...
function M:LOOT_OPENED(_, autoloot)
	...
	if items > 0 then
		for i=1, items do
			...
			local textureID, item, quantity, currencyID, quality, _, isQuestItem, questId, isActive = GetLootSlotInfo(i)

			if currencyID then
				item, textureID, quantity, quality = CurrencyContainerUtil.GetCurrencyContainerInfo(currencyID, quantity, item, textureID, quality)
			end
